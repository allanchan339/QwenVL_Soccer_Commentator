...
def debug_inpainting(
    ...
    Args:
        video_path (str): Path to the video or image.
        bbox_shift (int): Bounding box shift value.
        extra_margin (int): Extra margin for cropping.
        parsing_mode (str): Parsing mode ('jaw' or 'raw').
        left_cheek_width (int): Width for left cheek parsing.
        right_cheek_width (int): Width for right cheek parsing.

    Returns:
        tuple: (debug image as np.ndarray, info text as str)
    """
...
def download_model() -> None:
    """Check for required model files and prompt user to download if missing."""
    # 检查必需的模型文件是否存在
    required_models = {
...
def load_models_and_device(use_float16: bool = False):
    """Load all required models and set device and precision.

    Args:
        use_float16 (bool): Whether to use float16 precision.

    Returns:
        Tuple: (device, vae, unet, pe, weight_dtype, audio_processor, whisper, timesteps)
    """
...
(device, vae, unet, pe, weight_dtype, audio_processor, whisper, timesteps) = load_models_and_device(args.use_float16)
...
def check_video(video: str) -> str:
    """Check and convert video to target fps and naming convention if needed.
...
def create_gradio_ui(
    ...
    """Create and return the Gradio UI Blocks object.

    Args:
        vae, unet, pe, weight_dtype, audio_processor, whisper, timesteps, device: Model and utility objects.
